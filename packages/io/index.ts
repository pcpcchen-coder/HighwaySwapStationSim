...
function objectRows(rows: Record<string, unknown>[]): Cell[][] { if (!rows.length)
    return [['No rows']]; const keys = Object.keys(rows[0]); return [keys, ...rows.map(r => keys.map(k => { const v = r[k]; return v === null ? null : typeof v === 'number' ? v : typeof v === 'object' ? JSON.stringify(v) : String(v); }))]; }
