...
export function exportWorkbook(project: Project, result: RunResult, capacityRuns:CapacityRuns={}) {
    ...
    if(project.loadPlan){
      ...
      sheets.push({name:'Ten_Year_Settings',rows:[['energyValueCnyPerKWh',plan.energyValueCnyPerKWh],['pcsReferenceEfficiency',plan.pcsReferenceEfficiency??project.efficiency.pcs],['Conversion comparison',isSingleBus(project)?'同量DC交付：SST＋DC/DC vs 箱變＋PCS＋DC/DC；PCS容量足夠、下游相同；固定每度電價值；不含共同背景用電、投資、維運、稅務差額':'原方案加權效率比較'],['Basis',isSingleBus(project)?'第一期固定設備；營運日含需求日與背景日；三日平均扣除期初庫存消耗，不保證排隊穩定；能源後貢獻未扣CAPEX/OPEX/稅務':'代表日需求折算；加權效率比較同一DC售電量；節電收益未扣CAPEX/OPEX/稅務'],['Units','kWh / CNY; source annual units inferred as 萬度 / 萬元']]},
