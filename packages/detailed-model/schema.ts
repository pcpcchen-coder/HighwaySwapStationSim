...
export const detailedSchema=z.object({
 version:z.literal(1),
 topology:z.object({sharedDD:z.boolean(),sharingMode:z.enum(['SIMULTANEOUS','EXCLUSIVE']),pcsRatingKW:nonnegative,swapTerminalCounts:z.object({pcs:nonnegative.int().max(250),sst:nonnegative.int().max(250)}).strict().optional(),busTies:z.array(z.object({station,enabled:z.boolean(),direction:z.enum(['SST_TO_PCS','PCS_TO_SST']),kw:nullable}).strict()).max(2)}).strict(),
