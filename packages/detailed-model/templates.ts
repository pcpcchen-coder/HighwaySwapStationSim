...
export function appendDetailedTemplate(project:Project,config:DetailedConfig,path:ConfigPath):AddedDetailedTemplate{
 ...
 const blankBattery=(prefix:string,family:string)=>({id:id(prefix),family,capacityKWh:null,soh:null,initialSOC:null});
 const makeSlot=(rackId:string,family:string)=>({enabled:false,slot:{id:rackId,sink:rackId,battery:blankBattery(`${rackId}-battery`,family),chargeKW:null,chargeEfficiency:null}});
 switch(key){
  ...
  case 'service.swapArrivals':item={id:id('swap-arrival'),fleetId:'A-truck',atMinute:0,returnSOC:null,unitPrice:null,returnedPack:null};message='已新增換電事件骨架；第0分鐘是可編輯時間欄初值。請填回站SOC、售價與實際到站時間。';break;
