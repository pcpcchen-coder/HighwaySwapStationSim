...
type Shape='string'|'number'|'boolean'|{literal:readonly(string|number)[]}|{nullable:Shape}|{optional:Shape}|{array:Shape}|{[key:string]:Shape};
const nullable=(shape:Shape):Shape=>({nullable:shape}), array=(shape:Shape):Shape=>({array:shape}), optional=(shape:Shape):Shape=>({optional:shape});
const band:Shape={fromSOC:'number',toSOC:'number',voltageV:nullable('number'),maxKW:nullable('number'),maxCurrentA:nullable('number')};
const station:Shape={literal:['A','B']};
const battery:Shape={id:'string',family:'string',capacityKWh:nullable('number'),soh:nullable('number'),initialSOC:nullable('number'),energySOC:optional(nullable(array({soc:'number',energyFraction:nullable('number')})))};
...
const swap:Shape={id:'string',station,kind:{literal:['truck-swap','passenger-swap']},enabled:'boolean',bays:nullable('number'),swapMinutes:nullable('number'),readySOC:nullable('number'),slots:array({id:'string',sink:'string',battery,chargeKW:nullable('number'),chargeEfficiency:nullable('number'),chargeBands:optional(nullable(array(band)))})};
...
const swapArrival:Shape={allowedSlotIds:optional(nullable(array('string'))),id:'string',fleetId:'string',atMinute:'number',returnSOC:nullable('number'),unitPrice:nullable('number'),returnedPack:nullable(battery),requestedKWh:optional(nullable('number')),minReturnSOC:optional(nullable('number'))};
...
function check(input:unknown,s:Shape,path:string):void {
 ...
 if('optional'in s){if(input!==undefined)check(input,s.optional as Shape,path);return;}
 ...
 for(const [key,child] of Object.entries(s))if(!Object.hasOwn(value,key)){if(typeof child!=='object'||!('optional'in child))throw Error(`DRAFT_MISSING:${path}.${key}`);}else check(value[key],child,`${path}.${key}`);
}
