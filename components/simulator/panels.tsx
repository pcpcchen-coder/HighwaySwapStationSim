...
type Props={project:Project;setProject:(p:Project)=>void;result:RunResult};
...
export function FinancePanel({project,setProject,result}:Props){
 ...
 const projection=investmentCase(snapshot,result);
 const finance=projection?.projection;
 const reference=breakEven(4500,.3)!;
 const financeDraftChanged=JSON.stringify(f)!==JSON.stringify(snapshot.finance);
