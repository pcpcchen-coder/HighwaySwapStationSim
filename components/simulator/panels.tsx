...
export function StationPanel({project,setProject}:{project:Project;setProject:(p:Project)=>void}){const fields=[['batteries','站內電池數','顆'],['capacityKWh','電池額定容量','kWh'],['bays','換電工位','位'],['swapMinutes','每次換電時間','min'],['guns','充電槍數','支'],['gunKW','單槍上限','kW'],['chargePoolKW','充電共享功率','kW'],['batteryChargeKW','每顆電池補電上限','kW'],['auxiliaryKW','固定輔助負載','kW']] as const;return <div className="two-column">{(['A','B'] as const).map(s=><section className="panel" key={s}><div className="panel-heading"><h2>{s} 服務區 · 營運配置</h2><span className="pill">{project.engineering?'附件設備配置':'DEMO ASSUMPTION'}</span></div><SocRangeEditor project={project} station={s} setProject={setProject}/><hr/><div className="form-grid">{fields.map(([key,label,unit])=><NumberField key={key} label={label} unit={unit} value={project.station[s][key]} onChange={value=>{const next={...project,station:{...project.station,[s]:{...project.station[s],[key]:value??0}}};setProject(next.engineering?{...next,topology:engineeringTopology(next)}:next);}}/>)}</div><p className="notice">詳細案例預設 8 倉、513 kWh、SOC 20–100%、DD 560 kW。每側 1 工位 / 7.5 分鐘為對應 8 次每小時的作業假設。上方 SOC 套用保留拓撲；其餘站務欄位會重建拓撲。修改電池容量後，請再次套用 SOC 同步需求。</p></section>)}</div>;}
