...
import {LoadPresetControls} from './load-preset-controls';
import {demandSummary,splitDemand,DEMAND_KEYS} from '../../packages/load-planning/index.ts';
import {updateACServiceValue} from '../../packages/service-profile/index.ts';
...
export function ServicePanel({project,setProject}:{project:Project;setProject:(p:Project)=>void}){
 const [station,setStation]=useState<StationId>('A'),[selectedDay,setDay]=useState('0'),[mode,setMode]=useState<ImportMode>('merge'),[advanced,setAdvanced]=useState(false),[reading,setReading]=useState(false),[error,setError]=useState(''),[message,setMessage]=useState(''),[allocation,setAllocation]=useState(false);
 ...
 return <section className="panel">
  ...
  <LoadPresetControls project={project} day={day} onChange={next=>{setProject(next);setAllocation(true);setMessage('劇本已更新；請重新執行測算以同步能量流與结果。');}} onError={setError}/>
  <p className="panel-note">直接修改每天、每站、每小時的到站車次及總需求。kWh 為該小時所有到站車輛的合計需求，未分流時平均分配給每台車；有 AC/DC 分流時，各母線內按車次平均分配。實際交付量由受限模擬決定。修改後請重新測算。</p>
  ...
  {project.detailed&&!project.detailed.service.useHourlyTruckDemand&&<p className="notice">目前完整模型未啟用逐時重卡需求，這些列不會產生到站事件。載入任一負載劇本可啟用。</p>}
  ...
  <div className="plan-toolbar"><Button variant={allocation?'default':'outline'} onClick={()=>setAllocation(!allocation)}>{allocation?'返回需求與定價':'檢視／編輯 AC・DC 分流'}</Button>{allocation&&!rows.every(r=>r.ac)&&<Button variant="outline" onClick={()=>setProject({...project,services:project.services.map(r=>r.day===day&&r.station===station?{...r,ac:r.ac??{swapCount:0,swapKWh:0,chargeCount:0,chargeKWh:0}}:r)})}>本日啟用分流（AC 初值0）</Button>}</div>
  {allocation?<><p className="plan-note">總量及 AC 可編輯；DC＝總量－AC。AC/DC各自使用所屬電池艙與共用充電槍，仍共用原有換電工位。修改總量後須同步核對 AC 子集，不能超過總量。獨立超充不在此劇本需求範圍。</p><div className="table-scroll allocation-table"><Table><TableHeader><TableRow><TableHead rowSpan={2}>時間</TableHead><TableHead colSpan={4}>AC＋DC 總量</TableHead><TableHead colSpan={4} className="ac-col">AC／PCS（輸入）</TableHead><TableHead colSpan={4} className="dc-col">DC／SST（計算）</TableHead></TableRow><TableRow>{[0,1,2].flatMap(group=>['換電次數','換電 kWh','充電次數','充電 kWh'].map((name,i)=><TableHead key={`${group}-${i}`}>{name}</TableHead>))}</TableRow></TableHeader><TableBody>{rows.map(row=>{const parts=splitDemand(row);return <TableRow key={row.hour}><TableCell>{String(row.hour).padStart(2,'0')}:00</TableCell>{DEMAND_KEYS.map(key=><TableCell key={'total'+key}><Input aria-label={`${station} ${row.hour}時 總${key}`} type="number" min={0} step={key.endsWith('Count')?1:'any'} value={row[key]} onChange={e=>attempt(()=>{if(e.target.value==='')throw Error('欄位不可空白');setProject(updateServiceValue(project,station,day,row.hour,key,e.target.valueAsNumber));})}/></TableCell>)}{DEMAND_KEYS.map(key=><TableCell key={'ac'+key}>{row.ac?<Input aria-label={`${station} ${row.hour}時 AC ${key}`} type="number" min={0} step={key.endsWith('Count')?1:'any'} value={row.ac[key]} onChange={e=>attempt(()=>{if(e.target.value==='')throw Error('欄位不可空白');setProject(updateACServiceValue(project,station,day,row.hour,key,e.target.valueAsNumber));})}/>:<span>未分流</span>}</TableCell>)}{DEMAND_KEYS.map(key=><TableCell key={'dc'+key} className="dc-cell">{parts.dc?fmt(parts.dc[key],key.endsWith('Count')?0:2):'—'}</TableCell>)}</TableRow>;})}<TableRow><TableCell>合計</TableCell>{(['total','ac','dc'] as const).flatMap(part=>DEMAND_KEYS.map(key=><TableCell key={part+key}>{part==='total'||demandSummary(rows).routed?fmt(demandSummary(rows)[part][key],key.endsWith('Count')?0:2):'—'}</TableCell>))}</TableRow></TableBody></Table></div></>:<div className="table-scroll"><Table><TableHeader><TableRow><TableHead>時間</TableHead>{columns.map(([key,label])=><TableHead key={key}>{label}</TableHead>)}<TableHead>閒（原值）</TableHead></TableRow></TableHeader><TableBody>{rows.map(row=><TableRow key={row.hour}><TableCell className="mono">{String(row.hour).padStart(2,'0')}:00</TableCell>{columns.map(([key,label])=><TableCell key={key}><Input aria-label={`第 ${day+1} 天 ${station} 區 ${row.hour} 時 ${label}`} type="number" min="0" max={key.endsWith('Count')?1000:undefined} step={key.endsWith('Count')?1:'any'} value={row[key]} onChange={e=>attempt(()=>{if(e.target.value==='')throw Error('欄位不可空白；無需求請填 0。');setProject(updateServiceValue(project,station,day,row.hour,key,e.target.valueAsNumber));setMessage('');})}/></TableCell>)}<TableCell className="muted">{row.idleRaw??'未提供'}</TableCell></TableRow>)}</TableBody></Table></div>}
 </section>;
}
