...
export function VerificationPanel({result,busy,onRun}:{result:RunResult;busy:boolean;onRun:(project:Project)=>void}) {
 ...
 return <>
  <section className="panel"><div className="panel-heading"><div><p className="eyebrow">INDEPENDENT VERIFICATION / ENGINE 0.4.0</p><h2>三個案例，從元件一路對到結算。</h2></div><a href="/reports/HighwaySwapSim_Verification.pptx" download><Button variant="outline">下載 0.3.0 封存簡報 ↗</Button></a></div>
  ...
  <section className="panel mt-6"><h2>可報告的結論與下一個驗收門檻</h2><p className="panel-note">「在版本 0.4.0、明列測試輸入與結算規則下，三個多日案例已通過獨立解析比對；能量絕對差不超過 10⁻⁶ kWh，結算金額逐分一致。」</p><p className="panel-note">投入投資決策前，應以同步來源電表、各轉換器輸出、電池 SOC、逐筆服務及帳單，完成獨立留出期間的實測驗證，再納入 CAPEX、需量費、維護、換電池、稅與需求不確定性。量測誤差與接受門檻需先約定，不能把解析測試容差當成真實場站的保證。</p><a href="https://github.com/pcpcchen-coder/HighwaySwapStationSim/blob/main/docs/VERIFICATION.md" target="_blank" rel="noreferrer">完整驗證紀錄、首次不符及修正紀錄 ↗</a></section>
 </>;
}
