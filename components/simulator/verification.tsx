...
export function VerificationPanel({result,busy,onRun}:{result:RunResult|null;busy:boolean;onRun:(project:Project)=>void}) {
 ...
 return <>
  <section className="panel"><div className="panel-heading"><div><p className="eyebrow">INDEPENDENT VERIFICATION / ENGINE 0.4.1</p><h2>三個案例，從元件一路對到結算。</h2></div><a href="/reports/HighwaySwapSim_Verification.pptx" download><Button variant="outline">下載 0.3.0 封存簡報 ↗</Button></a></div>
   <p className="panel-note">獨立 Python Decimal 70 位精度解析計算，與正式 TypeScript 引擎比較。此頁的驗證案例採受控參數，完整設備範例另見上方「410.4 kWh 物理案例」。數值一致只適用已測輸入與明定規則，實際獲利還需要實測電量、合約電價及完整成本校準。</p>
