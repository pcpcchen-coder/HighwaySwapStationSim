...
export function ReferenceBackdrop({nodes}:{nodes:Equipment[]}){
 const singleBus=!nodes.some(n=>n.id.endsWith('-bus-pcs'));
 ...
 return <g className="reference-backdrop" aria-hidden="true" pointerEvents="none">
  ...
  <text x={REFERENCE_WIDTH/2} y={1090} textAnchor="middle" className="reference-bus-label">{singleBus?`公共 DC ${voltage('sst')} V 母線 · 雙向共享`:`DC ${voltage('sst')} V 母線 1 · SST 路徑`}</text>
  {!singleBus&&<text x={REFERENCE_WIDTH/2} y={1380} textAnchor="middle" className="reference-bus-label">DC {voltage('pcs')} V 母線 2 · PCS 路徑</text>}
