...
使用者可依日期、服務區、小時修改換電／充電到站車次、合計kWh及价格；可查看均勻或固定種子到站生成方式。新增需求專用 CSV／JSON 匯入匯出，輸出目前草稿所有天數兩區，與上次結果XLSX分清。CSV需支援Excel另存UTF-8後編輯回匯，day採1起算；格式及操作寫入SERVICE_PROFILE.md及USER_GUIDE.md。

以day/station/hour為鍵，支援部分列合併及全期取代；完整13欄保留來源總價與idleRaw null/0。整批校驗成功後一次更新services；拒絕缺欄、重複、越界、非有限值、不合法車次及超限總請求。不重建拓撲、不調整天數、不修改到站方式、種子或計價設定；保留原列順序避免改變SEEDED事件。

車次是到站事件數，kWh是該小時同類車輛合計需求；均分至每台車，實際交付受物理約束。来源總價不隨服務費自動改寫，來源價格模式需可檢查編輯。換電SOC衝突保留提示，只有使用者明確按本日SOC重算才更新該站該日需求。修改／匯入後先備份專案JSON、重新受限測算，再驗算與匯出結果XLSX。

驗收需涵蓋多日精確往返、部分合併與全期覆蓋、錯誤不部分套用、列順序與種子重現、獨立到站時間／每車kWh／收入／購電期望值，以及SOC顯式修正。有限測試不等同所有未測案例或真實獲利100%保證。


# 133. 個別設備參數、轉換效率與整套設定交換

逐台設備可編輯名稱、啟停及已建模的額定參數。SST、箱變、PCS、DD／超充堆提供可選的params.efficiency，畫面用百分比、存檔用(0,1]比例；缺省繼承同類全域值，明設值不跟隨全域變動。效率集中解析，不在UI另寫物理公式；箱變輸出容量使用kVA×PF×有效效率，SST/PCS/DD輸出額定不再乘自身效率。個別效率與SOURCE_CHAIN互斥，未知/非法/不可能的校準值不得靜默忽略。

新增整套設備JSON及設備參數CSV。JSON含節點、連線、全域/單台效率、站務、工程配置、期別、停機排程；保持陣列順序，不自動重建拓撲，保留目前需求、天數與財務。CSV以設備ID+欄位原子合併，包含效率空白繼承，可供Excel UTF-8修改。未知欄位、重複、非法值及不合法拓撲/排程整批拒絕；文字需保護試算表公式執行且可逆還原。

未實作的規格紀錄與實際計算參數須明確分開；電池容量/SOC目前依站配置，電纜長度不生成I²R，補償kvar不生成無功控制，乘用車規格不虛構服務收入。全域效率與附件對稱容量摘要不冒充手改拓撲結果。修改後先保存草稿、重新受限測算、能量流驗算、結果XLSX；重建拓撲會覆盖個別設定，操作文件須寫明。

驗證涵蓋覆寫/繼承、分支隔離、輸出額定、箱變共享輸入邊界、全設定與CSV精確往返、錯誤原子性、固定排程及順序、多日帳本/trace/XLSX一致。重跑三個既有獨立解析案例，保留首次失敗及更正原因。


# 134. 可設定換電 SOC 上下限（2026-09-09）

在設備與容量、站務配置提供每站獨立百分比上下限，0≤回收下限<交付上限≤100。按套用才原子更新該站SOC與全部日期的換電kWh=車次×容量×SOC差；此為對132章「只有本日重算可更改需求」的明確新增操作。既有本日重算、需求檔匯入及設備檔匯入契約不改。

SOC套用不可重建拓撲、排序服務列、覆蓋另一站、車次、直接充電、價格、停機排程或個別效率。已套用值與輸入预覽分清；結果仍須重新測算。初始庫存依交付SOC，換電回收依下限；乘用車與逐車SOC曲線仍不在此功能範圍。百分比僅用於顯示，存檔沿用0–1比例及schema1.3。

驗收需核對每站全期同步、非法值原子拒絕、不同SOC的三日獨立能量／庫存／來源小時結算、trace積分，以及專案JSON／設備JSON／需求CSV／結果XLSX的保存與還原。設備JSON匯入保留目前需求，需明確套用SOC才同步，不得靜默改寫。靜態容量表必須顯示目前SOC的計算量，不能持續顯示固定3370.84kW需求。


# 135. 原圖缺口與完整模型（2026-09-09）

新增 detailed.version=1，外層 schema1.3 保持相容。完整物理案例與新專案使用 v0.6；沒有 detailed 的歷史案保持原引擎，原始來源／獨立預期不改。完整操作與逐項原圖對照以 COMPLETE_MODEL_GUIDE.md 為準。本章更新133–134章原有未建模項目的範圍。

必須具備：16倉、DD共用的16支外槍與8支獨立超充槍；同側受控母聯及跨站共享；CATL乘用車完整事件／補電／收入；逐包容量、SOH、SOC及每車SOC/電壓/電流/溫度曲線；UPS/ATS/DCAC；PV/MPPT/棄光/受限外售；ESS充放、自放電及係數老化；分支電阻/電抗/壓降/載流/熱、負載效率與待機、箱變空載/銅損/補償、定時保護；分期投運、規則EMS與通訊降級。模型相互共享實際拓撲容量，不以新增槍位創造額外電源。

物理邊界須明確：分支送端電壓為指定值，不冒充全AC網路潮流或EMT。保護使用提供的故障電流與延時，分斷能力不足必須拒絕。爬升是最長1分鐘且在事件處切分的離散控制，限制依真正Δt。周轉電池SOH由初始／回站電池資料指定；不宣稱自動預測壽命。ESS係數老化與財務月度替換不等同現場BMS或實物更換。

# 136. 可編輯、缺值與交換

所有新增設定須有欄位、集合結構及完整JSON/CSV交換。未知值明確null，可保存未完成草稿；啟用模型的必要缺項須精確指到路徑，物理不足不執行，財務不足不捏造零成本淨利。曲線點亦可待填，啟用後必須完整、範圍足夠及單調輸入律可解。未知欄位與非法批次原子拒絕，不靜默剝除。

完整模型設定頁有本地草稿，套用才更新Project。重建設備連接是明確動作，會取代手動拓撲；單純測算不能自行重建。轉換器效率與儲能／PV模型不一致時阻擋並提示同步。整套設備JSON含detailed；完整模型CSV採JSON Pointer的path/type/value，保留array/object/null/empty/string精確語義。結果XLSX包含全部Project快照、新設備／服務／儲能／電氣／切換／光伏／外售／存貨／月帳單與現金流明細。

# 137. 完整財務邊界

逐來源逐時電價结算採既有CNY分規則。按真實日曆月分帳，15分鐘固定區間需量、容量費、超約與PF調整各自明列；不足全月不能冒充真實月峰。資產購入、租用、所有權、折舊、殘值、處置及替換月份逐項可填。人力／維保／場地等OPEX、簡化稅制、應收與營運資金、逐月營運假設均可保存匯入匯出。

周期加權能源庫存要區分外購與ESS到換電池的內部轉撥，合併時消除內部成本，避免重複列購電費。現金支出、能源費用與投資現金流分開。完整長期預測需逐月資料和明確確認；資料不全或IRR非唯一時顯示無法計算，不把數日結果直接冒充長年收益。

# 138. 同一時間窗的完整能量流驗算

圖面涵蓋全部節點，來源種類區分grid/PV/ESS/UPS。瞬時kW與累計kWh都要分外部輸入和儲能內部移轉。外部帳：電網＋光伏＝末端（含換電補電）＋外售＋網路損耗＋儲能被動損耗＋儲能增量。另用服務事件拆成換電淨交付及周轉庫存變化；不可同時再加補電量。

任意子分鐘窗口按原始trace积分，自放電按ODE計算，容量衰減溢出作離散步末調整，歸 from < t ≤ to 的前一區間；連續功率仍採[from,to)。缺帳不以0顯示通過。XLSX、圖面與JSON驗算報告只使用最後成功快照，改設計／事件／成本後必須重新測算。

# 139. 可重現測試與可報告結論

保留舊三例47,489項獨立預期。新增3日DD共享與母聯、4日PV/ESS/UPS、5日乘用車與跨月費用三例，獨立Python Decimal70產生期望，網站同引擎實跑；數值／時間、每節點／連線／來源表與庫存逐項對帳。網站提供固定案例執行、expected/actual/delta/tolerance及報告JSON；修改輸入後不能沿用固定案例的通過標記。

驗收還須包含800V車端曲線、停電、初始超溫、分數分鐘爬升、補償與箱變共同損耗、PV外售上限、不轉售電網、ESS內部成本消除、缺值、非法匯入以及XLSX回匯重跑一致。保留首次問題與修正原因，更新操作文件及PPT。結論限於聲明版本、受控輸入與容差內的一致性；有限測試不能保證任意未測設計或真實獲利100%正確。


## 140. 電池需求語意修正、逐艙 SOC 與回充曲線

0.7.0：逐時輸入 kWh 必須保留絕對需求，依實際電池容量、SOH、能量曲線與回站 SOC 下限配對，不可默默縮小需求；無法服務時保留未交付與零收入。逐車 SOC 模式與固定 kWh 模式互斥。每艙可設定 chargeBands；BatterySpec 可設定 energySOC 分段線性對照，省略/null 明示沿用固定功率／線性能量。缺測值可保存 null，啟用使用時檢核必要資料。

每個電池艙須保留隨模擬時刻的電池身份、SOC、庫存、待補量、實際輸入／入庫功率及狀態。任意分鐘按實際恆功率區段求值，不跨換電事件內插，不以 SOC 平均冒充終點值。當下、逐時、累計圖面均顯示游標狀態，累計模式明確標示終點 SOC。

新增參數須有逐項編輯、曲線骨架、JSON／CSV 交換與專案 XLSX 快照重現。結果 XLSX 增加 Battery_SOC_Trace；全圖 JSON 包含游標艙位狀態。獨立三日解析案例覆蓋分段、非線性能量映射、降額、停電與原先 SOH 反例。操作與邊界以 BATTERY_SOC_AUDIT.md 為準。


# 140. 雙側 DD 3:5 配置（2026-09-16）

新完整物理預設的 A/B 兩側各使用 PCS 3 台、SST 5 台 DD；各 8 艙。其餘實體配置不變，2026-09-17 補圖指定每側 PCS 1 台與 SST 2 台外接雙槍終端（共 6 槍），需同步節點、連線、服務槍位與資產清單；不得因 ceil(DD數/2) 自動增加槍。新 swapTerminalCounts 欄位需可編輯並保留於 JSON/CSV/XLSX；舊匯入檔案不自動重建或改值。圖面與總覽依實際饋線顯示分組，測算使用更新後連線。驗收見 DD_ALLOCATION_3_5.md。


# 2026-09-18 使用者確認：單母線第一期（0.8.0，優先於舊配置）
